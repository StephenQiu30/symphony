defmodule SymphonyElixir.CoreTest do
  ...
  test "app server starts with workspace cwd and expected startup command" do
    ...
    try do
      ...
      assert {:ok, _result} = AppServer.run(:codex, workspace, "Fix workspace start args", issue)
    ...
    try do
      ...
      for {runtime, binary, session_id} <- [
        ...
        assert {:ok, %{result: :turn_completed, session_id: ^session_id}} =
                 SymphonyElixir.AgentCli.run(runtime, workspace, "hello", issue, on_message: fn message -> send(parent, {:agent_message, runtime, message}) end)
